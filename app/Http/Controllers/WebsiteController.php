...
class WebsiteController extends Controller
{
    ...
    public function KgStudentCreate($id)
    {
        ...
        if ($student) {
            ...
            sms_send('01913366387', 'Your Username is ' . $student->username . ' and Password is ' . $password . '. Please login to following link to complete the admission form. Link: https://bafsd.edu.bd/login');
    ...
    public function admissionview(Request $request)
    {
        ...
        $admissiondata = AdmissionOpen::with(['class', 'version', 'session']);
        if (isset($request->test)) {
            $admissiondata = $admissiondata->where('session_id', $session->id)->where('class_id', 11)->get();
        } else {
            $admissiondata = $admissiondata->where('class_id', 11)->where('start_date', '<=', date('Y-m-d'))->where('end_date', '>=', date('Y-m-d'))
        ...
        return view('frontend-new.admissionlistxi', compact('admissiondata', 'session', 'notices', 'pages'));
    }
