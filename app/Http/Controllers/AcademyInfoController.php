...
class AcademyInfoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        Session::put('activesubmenu', 'act');

        $students = DB::table('students')
            ->join('student_activity', 'students.student_code', '=', 'student_activity.student_code')
            ->select('students.*', 'student_activity.*')
            ->where('students.student_code', Auth::user()->ref_id)
            ->get();

        $activities = $students->groupBy('session_id');
        $sessions = Sessions::pluck('session_code', 'id');
        $classes = DB::table('classes')
            ->join('student_activity', 'classes.class_code', '=', 'student_activity.class_code')
            ->select(
                'classes.class_name',
                'classes.class_code',
            ...
            ->whereIn('student_activity.student_code', $students->pluck('student_code'))
            ->get()
            ->groupBy(function ($item) {
                return $item->student_code . '-' . $item->session_id;
            })
            ->map(function ($group) {
                return $group->first();
            });
        ...
        $exams = Exam::whereIn('class_code', $students->pluck('class_code'))->whereIn('session_id', $sessions)->get();

        return view('report.academic_transcript.index', compact('exams', 'sessions', 'classes', 'session_id'));
    }
