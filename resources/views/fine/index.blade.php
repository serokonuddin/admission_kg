...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
       <h4 class="py-3 mb-4">
          <span class="text-muted fw-light">Dashboard /</span> Fine
       </h4>
       <div class="row mb-4">
          <!-- Browser Default -->
          <div class="col-md-12 mt-2">
             <div class="card">
                <h5 class="card-header">Fine Entry</h5>
                <div class="card-body">
                    @if (Auth::user()->getMenu('fine.store', 'name') || Auth::user()->group_id == 2)
                   <form  method="post" action="{{ route('fine.store') }}"  novalidate="" id="formsubmit">
                    @csrf
                    @else
                    ...
                    <input type="hidden" name="session_id" id="session_id" value="{{ $session->id }}" />
                      <div class="col-md-6 mb-3">
                         <label class="form-label" for="bs-name">Fine Name</label>
                         <input type="text" class="form-control" name="name"  id="name" placeholder="Fine Name" required="">
                         <div class="valid-feedback"> Looks good! </div>
                         <div class="invalid-feedback"> Please enter Fine Name. </div>
                      </div>
                      <div class="col-md-6 mb-3">
                         <label class="form-label" for="bs-name">Fine Amount</label>
                         <input type="text" class="form-control" name="amount"  id="amount" placeholder="Fine amount" required="">
                         <div class="valid-feedback"> Looks good! </div>
                         <div class="invalid-feedback"> Please enter Fine amount. </div>
                      </div>
                      <div class="col-md-6 mb-3">
          ...
          <div class="col-md mb-4 mb-md-0">
             <div class="card">
                <h5 class="card-header">Fine List</h5>
                    ...
                    <table class="table">
                      ...
                      <tbody class="table-border-bottom-0">
                        ...
                        <tr id="row{{ $fine->id }}">
                          ...
                          <td>{{ $fine->session->session_name }}</td>
                          <td>
                          {{ $fine->name }}
                          </td>
                          <td>
                          ...
                          <td>
                           <div class="dropdown">
                                ...
                                <div class="dropdown-menu" style="">
                                     @if (Auth::user()->getMenu('fine.edit', 'name') || Auth::user()->group_id == 2)
                                    <a class="dropdown-item edit"
                                    data-id="{{ $fine->id }}"
                                    data-session_id="{{ $fine->session_id }}"
                                    data-name="{{ $fine->name }}"
                                    data-amount="{{ $fine->amount }}"
                                    data-unit="{{ $fine->unit }}"
                                    data-unit_value="{{ $fine->unit_value }}"
                                    data-status="{{ $fine->status }}"
                                    href="javascript:void(0);"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                                    @endif
                                     @if (Auth::user()->getMenu('fine.destroy', 'name') || Auth::user()->group_id == 2)
                                    <a class="dropdown-item delete" data-url="{{ route('fine.destroy', $fine->id) }}" data-id="{{ $fine->id }}"  href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>
                                    @endif
                                </div>
                            </div>
                          </td>
