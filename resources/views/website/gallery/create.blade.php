    ...
    <div class="container-xxl flex-grow-1 container-p-y">
       ...
       <div class="row">
          <div class="col-md-12">
             ...
             <div class="card mb-4">
                <h5 class="card-header">gallery Information</h5>
                <!-- Account -->


                <div class="card-body">
                   <form id="formAccountSettings" method="POST"  action="{{route('gallery.store')}}">
                    <input type="hidden" value="{{isset($gallery)?$gallery->id:''}}" name="id" />
                    <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
                      <div class="row">
                        <div class="mb-3 col-md-12">
                            <label for="job_type" class="form-label">gallery Type</label>
                            <select id="job_type" name="type_id" id="type_id"  class="select2 form-select" required="required">
                               <option value="0">Select gallery Type</option>
                               @foreach($types as $type)
                               <option value="{{$type->id}}" {{(isset($gallery) && $gallery->type_id==$type->id)?'selected="selected"':''}}>{{$type->title}}</option>
                               @endforeach
                         ...
                         <div class="mb-3 col-md-12">
                            ...
                            <input class="form-control" type="text" id="title" name="title" required="required" value="{{isset($gallery)?$gallery->title:''}}" placeholder="gallery Name" autofocus="">
                         </div>


                         <div class="mb-3 col-md-12">
                            <label for="permanent_addr" class="form-label">Details</label>
                            <textarea type="text" class="form-control" id="details" name="details"  placeholder="Details" >{!!isset($gallery)?$gallery->details:''!!}</textarea>
                         </div>
                         <div class="mb-3 col-md-12 form-group gallery" id="photo_gallery">
                                <label for="inputPhoto" class="col-form-label">Photo <span class="text-danger">*</span></label>
                                <div class="input-group">

                                <input id="thumbnail" class="form-control" type="text" name="file" value="{{(isset($gallery))?$gallery->file:''}}">
                         ...
                         <div class="mb-3 col-md-6">
                            ...
                            <input class="form-control" type="date" id="publish_date" required="required" name="publish_date" value="{{isset($gallery)?$gallery->publish_date:''}}" placeholder="Publish Date" autofocus="">
                         </div>

                         <div class="mb-3 col-md-4">
                           <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                              <input type="radio" class="btn-check" name="active" id="active1" value="1" required="required" {{(isset($gallery) && $gallery->active==1)?'checked="checked"':''}} autocomplete="off" >
                              <label class="btn btn-outline-primary" for="active1">Active</label>

                              <input type="radio" class="btn-check" name="active" id="active0" value="0" required="required" {{(isset($gallery) && $gallery->active==0)?'checked="checked"':''}} autocomplete="off" >
                      ...
                      <a type="reset" href="{{route('gallery.index')}}" class="btn btn-outline-secondary">Cancel</a>
                   </form>
                </div>
                <!-- /Account -->
             </div>
