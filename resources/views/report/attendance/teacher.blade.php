...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y" id="attendance-part">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form class="needs-validation" method="post" action="{{ route('teacherAttendanceStore') }}"
                        ...
                        <ul class="nav nav-pills flex-column flex-md-row mb-3">
                            <li class="nav-item type college" data-type="college">
                                <a class="nav-link {{ $type == 3 || $type == '' ? 'active' : '' }}"
                                    href="javascript:void(0);"><img src="{{ asset('public/image/college.png') }}">
                                    College</a>
                            </li>
                            <li class="nav-item secondary type" data-type="secondary">
                                <a class="nav-link {{ $type == 2 ? 'active' : '' }}" href="javascript:void(0);"><img
                                        src="{{ asset('public/image/secondary.png') }}"> Secondary</a>
                            <li class="nav-item primary type" data-type="primary">
                                <a class="nav-link {{ $type == 1 ? 'active' : '' }}" href="javascript:void(0);"><img
                    ...
                    <a class="nav-link university" href="javascript:void(0);"><img src="{{asset('public/image/university.png')}}"> English</a>
                            ...
                            <li class="nav-item ">
                                <a class="nav-link datacync" href="javascript:void(0);"><img
                                        src="{{ asset('public/image/sync.png') }}"> Data Sync</a>
                            </li>
                        </ul>
                        <div class="card mb-4">
                            <div class="card-body">
                                    ...
                                    <div class="col-md">
                                        ...
                                        <div class="form-check form-check-inline mt-3">
                                            <input class="form-check-input" type="radio" checked="checked"
                                                name="version_id"
                                                {{ Session::get('version_id_t') == 1 ? 'checked="checked"' : '' }}
                                                id="bangla" value="1">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="version_id" id="english"
                                                {{ Session::get('version_id_t') == 2 ? 'checked="checked"' : '' }}
                                                value="2">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input type="text"
                                                class="form-control flatpickr-validation flatpickr-input active"
                                                id="attendance_date" name="attendance_date"
                                                value="{{ Session::get('attendance_date_t') ? Session::get('attendance_date_t') : date('Y-m-d') }}"
                                                placeholder="Attendance Date" required>
                                        </div>
                                        <div class="form-check form-check-inline mt-3">
                                            <input class="form-check-input" type="radio" checked="checked" name="shift_id"
                                                id="Morning" {{ Session::get('shift_id_t') == 1 ? 'checked="checked"' : '' }}
                                                value="1">
                                            <label class="form-check-label" for="Morning">Morning</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="shift_id" id="Day"
                                                {{ Session::get('shift_id_t') == 2 ? 'checked="checked"' : '' }}
                                                value="2">
                                            ...
                                            <select id="session_id" name="session_id" class=" form-select" required>
                                                ...
                                                @foreach ($sessions as $key => $session)
                                                    <option value="{{ $session->id }}"
                                                        {{ Session::get('session_id_t') == $session->id ? 'selected="selected"' : '' }}>
                                            ...
                                            <select id="class_id" name="class_id" class=" form-select" required>
                                                ...
                                                @foreach ($classess as $key => $class)
                                                    <option value="{{ $class->class_code }}"
                                                        {{ Session::get('class_id_t') == $class->id ? 'selected="selected"' : '' }}>
                                            ...
                                            <select id="section_id" name="section_id" class=" form-select">
                                                ...
                                                @foreach ($sections as $key => $section)
                                                    <option value="{{ $section->id }}"
                                                        {{ Session::get('section_id_t') == $section->id ? 'selected="selected"' : '' }}>
    ...
    <script type="text/javascript">
        $(function() {
            $(document.body).on('click', '.teacherdetails', function() {
                $.LoadingOverlay("show");
                var id = $(this).data('id');
                var url = "{{ route('getTeacherDetails') }}";
                $.ajax({
                    type: "post",
                    headers: {
        ...
        $(function() {
            ...
            $(document.body).on('change', '#section_id', function() {
                ...
                if (section_id && attendance_date && version_id && shift_id && session_id && class_id) {
                    var url = "{{ route('getTeachersReport') }}";
                    $.ajax({
                        type: "get",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')
