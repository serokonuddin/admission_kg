                                    ...
                                    @foreach ($students as $k => $student)
                                        ...
                                        @for ($key = 0; $key < 9; $key++)
                                            @php
                                                ...
                                                $total = ($cq_total ?? 0)+($mcq_total ?? 0);
                                                $conv_total = ($total)?round($total*80/100):0;
                                                $ct_conv_total = $conv_total+$ct;
                                                ...
                                                $practicalpassmark = $term_marks
                                                    ...
                                                    ->first()?->pass_mark;
                                                $totalpassmark = 40;
                                                if ($subjectId == 9 || $subjectId == 59 || $subjectId == 62) {
                                                    $totalpassmark = 20;
                                                }
                                                $isn=0;
                                                if($class==6){
                                                    ...
                                                    if(($iscq?->pass_mark) > $ct){
                                                            $isn=-1;
                                                    }
                                                    if($cqpassmark > $cq_total){
                                                            $isn=-1;
                                                    }
                                                    if($mcqpassmark > $mcq_total){
                                                             $isn=-1;
                                                    }
                                                }
                                                
                                                if($totalpassmark<$ct_conv_total){
                                                    $grade=getGrade(getPoint($ct_conv_total,1),1);
                                                }elseif($totalpassmark>=$ct_conv_total){
                                                    $grade=getGrade(getPoint($ct_conv_total,$isn),$isn);
                                                }else{
                                                    $grade='F';
                                                }
                                            @endphp
                                            @if ($key < 4)
                                                <tr>
                                                    ...
                                                    <!-- CT2 Total Column -->
                                                    <td
                                                        style="text-align: center; {{ $cqpassmark > $cq_total ? 'color: red; text-decoration: underline;' : '' }}">
                                                        {{ $iscq ? $cq_total ?? '' : '-' }}
                                                    </td>
                                                    <td
                                                        style="text-align: center; {{ $mcqpassmark > $mcq_total ? 'color: red; text-decoration: underline;' : '' }}">
                                                        {{ $ismcq ? $mcq_total ?? '' : '-' }}
                                                    </td>
                                                    <td
                                                        style="text-align: center; {{ $totalpassmark > $total ? 'color: red; text-decoration: underline;' : '' }}">
                                                        {{ $total ?? '' }}
                                                    </td>
                                                    ...
                                                    <!-- Total Marks Column -->
                                                    <td
                                                        style="text-align: center; {{ $totalpassmark > $ct_conv_total ? 'color: red; text-decoration: underline;' : '' }}">
                                                        {{ $ct_conv_total ?? '' }}
                                                    </td>
                                                    <!-- Grade Column -->
                                                    <td
