            ...
            <div class="panel-body">
                <div id="StudentAcademicTranscriptDiv">
                                                    ...
                                                    <table cellpadding="0" cellspacing="0" class="tableCenter tableBorderNo">
                                                        <tbody>
                                                            ...
                                                            <td style="width:70%; text-align:center; padding:0px 20px 0px 20px;">
                                                                ...
                                                                <h3 class="text-center" style="color:red; margin-top: 5px; margin-bottom: 0px; font-size:20px; font-weight:bold; white-space: nowrap;">Academic Transcript</h3>
                                                            </td>
                                    ...
                                    <div class="printHead" style="font-family:'Times New Roman, Times, serif';  margin-top:5px;">
                                        <div class="mobileScreenInfo">
                                            <div style="">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td style="width:40%;" colspan="2">Student's Name :{{ $student->first_name ?? '' }}</td>
                                                        <td style="width:60%;" colspan="3">Class Teacher : {{ $classteacher->employee_name ?? '' }} ({{ $classteacher->mobile ?? '' }})</td>

                                                    </tr>
                                                    <tr>
                                                        <td >Version: {{ $student->studentActivity->version->version_name ?? '' }}</td>
                                                        <td >Class : {{ $student->studentActivity->class_code }}</td>
                                                        <td >Section: {{ $student->studentActivity->section->section_name ?? '' }}</td>
                                                        <td >Roll : {{ $student->studentActivity->roll ?? '' }}</td>
                                                        <td >Year : {{ $student->studentActivity->session->session_name ?? '' }}</td>
                                        ...
                                        <div class="mobileScreen">
                                            <table class="resultDetailsTable" width="100%" cellspacing="0" cellpadding="0" style="min-width:570px; border-bottom:1px solid black;border-right:1px solid black;">
                                                ...
                                                <tr style="font-weight:bold;">
                                                    ...
                                                    <td colspan="{{ $markforcount + 6 }}" style="text-align:center;">{{ $exam->exam_title }}</td>
                    ...
                    @foreach ($studentdata as $key1 => $studen)
                        ...
                        @if (
                            ...
                            <tr>
                                ...
                                <td style="text-align:center;">{{ $studen->subject_id == 9 ? 50 : 100 }}</td>
                                @foreach ($subjectsmapping as $key => $subject)
                                    @if ($subject == 0)
                                        <td style="text-align:center;">
                        ...
                        @elseif(
                            ...
                            <tr>
                                ...
                                @php
                                    ...
                                    $ct1 = ($studen->ct1 ?? 0) + ($studentdata[$key1 - 1]->ct1 ?? 0);
                                    $ct2 = ($studen->ct2 ?? 0) + ($studentdata[$key1 - 1]->ct2 ?? 0);
                                    $ct3 = ($studen->ct3 ?? 0) + ($studentdata[$key1 - 1]->ct3 ?? 0);
                                    $ct = ($studen->ct ?? 0) + ($studentdata[$key1 - 1]->ct ?? 0);
                                    $cq_total = ($studen->cq_total ?? 0) + ($studentdata[$key1 - 1]->cq_total ?? 0);
                                    $mcq_total = ($studen->mcq_total ?? 0) + ($studentdata[$key1 - 1]->mcq_total ?? 0);
                                    $practical_total =
                                        ($studen->practical_total ?? 0) +
                                        ($studentdata[$key1 - 1]->practical_total ?? 0);
                                    $pair_total_total =
                                        ($subjectHighestMark[$studen->subject_id][0]->highest_mark ?? 0) +
                                        ($subjectHighestMark[$studentdata[$key1 - 1]->subject_id][0]->highest_mark ??
                                            '');
                                ...
                                <td style="text-align:center;" class="{{ $studen->gpa_point == 0 ? 'redtext' : '' }}">
                                    {{ $studen->gpa }}
                                </td>
                                <td style="text-align:center;" class="{{ $studen->gpa_point == 0 ? 'redtext' : '' }}">
                                    {{ $studen->gpa_point }}
                            ...
                            <tr>
                                ...
                                @php
                                    ...
                                    if ($gpa_point != -1) {
                                        ...
                                        if ($studen->is_fourth_subject == 1) {
                                            if ($studen->gpa_point > 2) {
                                                $gpa_point += (float) ($studen->gpa_point - 2);
                                            }
                                        } else {
                                            $gpa_point += (float) $studen->gpa_point;
                                        }
                    ...
                    @php
                        if ($gpa_point == -1) {
                            ...
                            $point = round($gpa_point / (count($studentdata) - 3), 2);
                        }
                    @endphp
                    <tr style="font-weight:bold;">
                        <td colspan="{{ $markforcount - 1 }}">
                            Grand Total Marks and GPA
                        </td>

                        <td></td>
                        <td></td>
                        <td style="text-align:center;">{{ $total ?? '' }}</td>
                        <td style="text-align:center;" class="{{ $isnegative == -1 ? 'redtext' : '' }}">
                    ...
                    <tr style="font-weight:bold;">
                        ...
                        <td></td>
                        @php
                            $percentage = round(($total * 100) / (count($studentdata) * 100 - 50));
                        @endphp
                ...
                <div style="padding-top:10px; padding-bottom:5px">
                    <table class="resultDetailsTable" cellpadding="0" cellspacing="0"
                        ...
                        <tr>
                            <td style="text-align:center;">
                                {{ $isnegative == -1 ? '' : $student->totalmark->position_in_section ?? '' }}</td>
                            <td style="text-align:center;height: 40px;">
                                {{ $isnegative == -1 ? '' : $student->totalmark->position_in_class ?? '' }} </td>
                        ...
                        <tr>
                            ...
                            <td rowspan="5" style="text-align:center;width: 30%!important">
                                @if ($student->version_id == 1 && $student->class_code > 5 && $student->class_code < 11)
                                    <img src="{{ asset('public/obayed ullah.png') }}" style="height: 70px;" />
                                @else
                                    <img src="{{ asset('public/humaira_vp_signature.jpg') }}"
                                        style="height: 70px;" />
                                @endif </br>
                                Vice Principal
                            </td>
                            <td rowspan="5" style="text-align:center;width: 30%!important"><img
                                    src="{{ asset('public/psi.jpg') }}" style="height: 70px;" /><br />Principal</td>

                        </tr>
                        <tr>

                            <td style="text-align:left;">Total Present :</td>
                            <td style="text-align:center;">{{ $student->studentExamAttendance->total_attendance ?? '' }}
                            </td>
...
<script type="text/javascript">
    function printElem(divId) {
        ...
        mywindow.document.write('<style>@page { size: A4; margin: 250px 5mm 5mm 5mm; }</style>');
