...
@endphp
        ...
        <table class="table table-striped table-no-bordered">
            <tr>
                ...
                <p>&nbsp;&nbsp;&nbsp;&nbsp;Class {{$classroman[$student->class_id??'']}}, Session: {{(int)$session->session_name+1}}</p>
                </td>
                
            </tr>
        </table>
        <table class="table table-striped table-no-bordered baf{{$student->category_id??''}} shift{{$student->shift_id??''}} version{{$student->version_id??''}}">
            <tr style="border: 2px solid #c0c0c0 !important">
                <td class="text-center">Shift: {{($student && $student->shift_id==1)?'Morning':'Day'}}</td>
                <td class="text-center">Version: {{($student && $student->version_id==1)?'Bangla':'English'}}</td>
        ...
        <table class="table table-striped table-no-bordered ">
            <tr >
                ...
                <td style="float: right;padding:0px;text-align: right" ><img src="{{asset('public/psi.jpg')}}" style="width: 90px;" /></td>
            </tr>
            <tr >
                <td style="float: left;padding:0px">
                    @if($student)
                    <a href="{{url('admissionPrint/'.$student->temporary_id.'/0')}}" target="_blank" class="btn btn-primary findAdmitcard"  style="background-color: #00ADEF">Print Admit Card</a>
                    <a href="{{url('admissionPrint/'.$student->temporary_id.'/1')}}" target="_blank" class="btn btn-primary findAdmitcard"  style="background-color: green">Save Admit Card</a>
                    @endif
                </td>
