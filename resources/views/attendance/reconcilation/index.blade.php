    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="card">
                        ...
                        <div class="col-sm-12 col-md-12">
                            <div class="row g-3 searchby">
                                    ...
                                    <select id="version_id" name="version_id" class="form-select"
                                        ...
                                        @foreach ($versions as $version)
                                            <option value="{{ $version->id }}"
                                                @if (!empty($activity) && isset($activity->version_id) && $activity->version_id == $version->id) selected="selected" @endif>
                                    ...
                                    <select id="shift_id" name="shift_id" class="form-select"
                                        ...
                                        @foreach ($shifts as $shift)
                                            <option value="{{ $shift->id }}"
                                                {{ !empty($activity) && isset($activity->shift_id) && $activity->shift_id == $shift->id ? 'selected="selected"' : '' }}>
                                                {{ $shift->shift_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select id="class_code" name="class_code" class="form-select"
                                        ...
                                        @foreach ($classes as $class_code => $class_name)
                                            <option value="{{ $class_code }}"
                                                {{ !empty($activity) && isset($activity->class_code) && $activity->class_code == $class_code ? 'selected="selected"' : '' }}>
                                                {{ $class_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select id="section_id" name="section_id" class="form-select"
                                        ...
                                        @foreach ($sections as $key => $section)
                                            <option value="{{ $section->id }}"
                                                {{ !empty($activity) && isset($activity->section_id) && $activity->section_id == $section->id ? 'selected="selected"' : '' }}>
                                ...
                                <div class="col-sm-3">
                                    ...
                                    <input type="date"
                                        class="form-control @if (Auth::user()->group_id != 3) flatpickr-validation flatpickr-input @endif"
                                        id="end_date" value="{{ $reconcilationDate->submit_date }}"
                                        @if (Auth::user()->group_id == 3) disabled="disabled" @endif name="end_date"
                                        required placeholder="yyyy-mm-dd">
                                </div>
                ...
                {{-- Table --}}
                @php
                    $today = \Carbon\Carbon::today(); // Get today's date using Carbon
                    $submitDate = \Carbon\Carbon::parse($reconcilationDate->submit_date);
                    // @dd($submitDate, $today);
                @endphp

                @if ($submitDate >= $today)
                    <!-- Display the form and button if submit_date is greater than or equal to today's date -->
                ...
                @else
                    <!-- Show a message if submit_date is less than today's date -->
                    <div class="alert alert-warning text-center">
                        The reconciliation period is not available yet. Please try again later.
                    </div>
                @endif
