...
<table class="table ">
    <thead>
        <tr>
            ...
            <th style="text-align: center">CT + PT</th>
            <th style="text-align: center">Term Exam</th>
            <th style="text-align: center">Conv 80% of Term Exam</th>
            <th>Total Mark </th>
            <th>GPA Point</th>
            <th>GPA</th>
            <th>Is Absent</th>
            ...
            <tr class="{{ $is_absentclass == 1 ? 'isabsent' : '' }}">
                ...
                <td>
                    <input class="form-control marks ct1   text-center " value="{{ $subjectmark->ct1 ?? '' }}"
                        ...
                        data-maxvalue="{{ $max_ct }}" data-marks_for="" type="number" min="0">
                </td>
                <td>
    ...
    $(document).ready(function() {
        ...
        function calculateSums() {
            ...
            if (parseInt(subject_id) !== 39) {
                convertedCq = Math.round(ct2 * 0.8);
            }

            let convertedSum = 0;

            if (parseInt(subject_id) === 39) {
                convertedSum = (ct2 / 25) * 100;
                $('input[name="totalmark' + student_code + '"]').val(ct2);
            } else {
