...
<script>
    ...
    $(document).ready(function() {
        ...
        function calculateSums() {
            ...
            $('input[name="totalmark' + student_code + '"]').val(cqmcqTotal);


            if (parseInt(subject_id) == 9) {
                cqmcqTotal = cqmcqTotal * 2;
            }

            let gpaPoint = is_pass ? calculateGpaPoint(cqmcqTotal) : 0;
            $('input[name="gpapoint' + student_code + '"]').val(gpaPoint);

            // Calculate GPA based on 19th TD input value and set in 21st TD input
            let gpa = is_pass ? calculateGpa(cqmcqTotal) : 'F';
