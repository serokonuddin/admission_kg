            ...
            @foreach ($students as $student)
                ...
                <tr class="{{ $is_absentclass == 1 ? 'isabsent' : '' }}">
                    <td data-label="SL" data-sl="{{ $loop->index + 1 }}" data-name="{{ strtoupper($student->first_name) }}"
                        ...
                        <input class="form-control marks text-center" value="{{ $subjectmark->ct1 ?? '' }}"
                            data-student_code="{{ $student->student_code }}" data-maxvalue="{{ $max_mark }}"
                            name="ct11{{ $student->student_code }}" id="ct1{{ $student->student_code }}"
                            type="hidden" min="0" max="{{ $max_mark }}" />
                        <input class="form-control marks text-center" value="{{ $subjectmark->ct2 ?? '' }}"
                            data-student_code="{{ $student->student_code }}" data-maxvalue="{{ $max_mark }}"
                            name="ct22{{ $student->student_code }}" id="ct2{{ $student->student_code }}"
                            type="hidden" min="0" max="{{ $max_mark }}" />

                    </td>
                    <td data-label="Student">{{ $student->first_name }}</td>
                    <td data-label="Roll">{{ $student->roll }}</td>
                    @if ($exam_type == 'ct1')
                        <td data-label="CT 1 ({{ $max_mark }})">
                            <input class="form-control marks text-center" value="{{ $subjectmark->ct1 ?? '' }}"
                                data-student_code="{{ $student->student_code }}" data-maxvalue="{{ $max_mark }}"
                                name="ct1{{ $student->student_code }}" id="ct1{{ $student->student_code }}"
                                type="number" min="0" max="{{ $max_mark }}" />
                        </td>
                    @elseif ($exam_type == 'ct2')
                        <td data-label="CT 2 ({{ $max_mark }})">
                            <input class="form-control marks text-center" value="{{ $subjectmark->ct2 ?? '' }}"
                                data-student_code="{{ $student->student_code }}" data-maxvalue="{{ $max_mark }}"
                    ...
                    @endif
                    <td class="total_none" data-label="Total Mark">
                    ...
                    <td data-label="Is Absent">
                        <input class="is_absent" {{ $is_absentclass == 1 ? "checked='checked'" : '' }} value="1"
                            name="is_absent{{ $student->student_code }}"
                            data-student_code="{{ $student->student_code }}" data-maxvalue="{{ $max_mark }}"
                            type="checkbox" />
                    </td>
                </tr>
...
<script>
    ...
    function calculateSums() {
        ...
        // NEW From Rezwan
        let ct1 = parseFloat($('input[name="ct1' + student_code + '"]').val()) || parseFloat($('input[name="ct11' +
                student_code + '"]').val()) ||
            0; // Safeguard against NaN
        let ct2 = parseFloat($('input[name="ct2' + student_code + '"]').val()) || parseFloat($('input[name="ct22' +
            student_code + '"]').val()) || 0;
        let sum1 = ct1 + ct2;
        ...
        let data = {
            ...
            ct1: $(`input[name='ct1${student_code}']`).val() || $(`input[name='ct11${student_code}']`).val(),
            ct2: $(`input[name='ct2${student_code}']`).val() || $(`input[name='ct22${student_code}']`).val(),
            ct3: '',
            ct4: '',
            ct: '',
            min1: '',
